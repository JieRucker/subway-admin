...
<script>
  import ruleNature from "./modal/rule-nature.vue";
  import ruleAddNature from "./modal/rule-add-nature.vue";

  export default {
    name: "rule-list",
    components: {
      ruleNature, ruleAddNature
    },
    data() {
      return {
        table: {
          header: [
            ...
            },
            {
              ...
              width: 180,
              render: (h, params) => {
                return h('span', {}, this._operateTypeName(params.row.operateType, params.row.score))
              }
            },
            {
              title: '操作',
                  ...
                  h('a', {
                    ...
                    on: {
                      click: () => {
                        ...
                        this.currentModal = this.modalDialog['ruleNature'];
                      }
                    }
                  }, '编辑'),
                ]);
        ...
        modalDialog: {
          ruleNature: 'Rule-Nature',
          ruleAddNature: 'Rule-Add-Nature'
        }
      }
    },
    computed: {
    ...
    methods: {
      ...
      _getUserList() {
        this.table.body = [];
        this.$api.ruleInterface.listRule({
      ...
      _deleteBatch() {
        this.$refs.userTable.getSelection().length ? (() => {
          let a = [];
          this.$refs.userTable.getSelection().forEach(m => a.push(m.userScoreRuleId));
          this.$Modal.confirm({
            title: '提示',
            content: '<p>确定删除？</p>',
            onOk: () => {
              this.$api.ruleInterface.removeRuleList({
                userScoreRuleIdJsonArray: JSON.stringify(a)
              }).then(res => {
                res = res.data;
                if (res.success) {
      ...
      _addBatch() {
        ...
        this.currentModal = this.modalDialog['ruleAddNature'];
      },
