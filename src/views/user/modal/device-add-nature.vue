...
<template>
  <Modal ref="modal" v-model="show" width="500" class="device-nature" :mask-closable="false"
    ...
    <p slot="header" class="header">
      <span class="header--title">添加设备</span>
    </p>
    <div class="body">
      <div class="body__form body__basic">
        <Form label-position="left">
          ...
          <FormItem label="设备序列号：" :label-width="80">
            <Input v-model="initial.sn" placeholder="请输入设备sn" style="width: 200px"></Input>
          </FormItem>
          <FormItem label="设备ip：" :label-width="80">
            <Input v-model="initial.databusIp" placeholder="请输入设备ip" style="width: 200px"></Input>
          </FormItem>
...
<script>
  ...
  export default {
    ...
    methods: {
      _save() {
        ...
        console.log(param);
        this.$api.deviceInterface.addDevice(param)
          .then(res => {
            res = res.data;
            if (res.success) {
  ...
  class ToolFunctor {
    ...
    _valid() {
      ...
      valid = true;
      params = ['deviceName','sn','databusIp','deviceType'];
      toastMSG = ['请输入设备名称','请输入设备序号','请输入设备ip','请选择设备类型'];
