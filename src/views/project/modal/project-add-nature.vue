...
<template>
  <Modal ref="modal" v-model="show" width="500" class="group-nature" :mask-closable="false"
      ...
      <div class="body__form body__basic">
        <Form label-position="left">
          <FormItem label="项目名称：" :label-width="80">
            <Input v-model="initial.projectName" placeholder="请输入项目名称" style="width: 200px"></Input>
          </FormItem>
          <FormItem label="项目描述：" :label-width="80">
            <Input v-model="initial.projectDesc" type="textarea" :rows="4" placeholder="请输入项目描述" style="width: 200px;"></Input>
          </FormItem>
          <FormItem label="城市编号：" :label-width="80">
            <Input v-model="initial.weatherCityId" placeholder="请输入项目所在城市编号" style="width: 200px;"></Input>
          </FormItem>
        </Form>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      _save() {
        ...
        this.$api.projectInterface.saveProject(param)
          .then(res => {
            res = res.data;
            if (res.success) {
  ...
  class ToolFunctor {
    ...
    _valid() {
      ...
      valid = true;
      params = ['projectName','projectDesc','weatherCityId'];
      toastMSG = ['请输入项目名称','请输入项目描述','请输入项目所在城市编号'];
    ...
    _set() {
      let DEFAULTS, target, initial;
      let source = new Object(null);
      initial = {...this.tis.initial};
