...
<template>
  <Modal ref="modal" v-model="show" width="500" class="group-nature" :mask-closable="false"
    ...
    <p slot="header" class="header">
      <span class="header--title">添加工种</span>
    </p>
    <div class="body">
      <div class="body__form body__basic">
        <Form label-position="left">
          <FormItem label="工种名称：" :label-width="80">
            <Input v-model="initial.groupName" placeholder="请输入工种名称" style="width: 200px"></Input>
          </FormItem>
        </Form>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      _save() {
        ...
        this.$api.groupInterface.addGroup(param)
          .then(res => {
            res = res.data;
            if (res.success) {
